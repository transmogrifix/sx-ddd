...
type
  ...
  public
    ...
    constructor Create(aRest: TRest; aSMTPServer: TSMTPServer;
  ...
  TSQLRecordEmailer = class(TOrmTimed)
  private
    fSender: RawUTF8;
...
begin
  ...
  if not IdemPChar(pointer(Res),pointer(Answer)) then
    raise ENetSock.CreateFmt('returned [%s], expecting [%s]',[Res,Answer]);
end;
...
class procedure TSQLRecordEmailer.InitializeTable(const Server: IRestOrmServer; const FieldName: RawUtf8; Options: TOrmInitializeTableOptions);
begin
  inherited;
...
begin
  ...
  result := fDaemon.Rest.Orm.Update(fPendingTask,'State,SendTime');
end;
...
begin
  ...
  body := AlgoSynLZ.Decompress(pendingEmail.MessageCompressed);
  result := length(body);
  ...
  fDaemon.Rest.Orm.Update(pendingEmail,'State,ErrorMsg'); // always write
end;
...
constructor TDDDEmailerDaemon.Create(aRest: TRest);
begin
...
{  result := CheckRecipients(aRecipient);
  ...
  try
    ...
    {$endif}
    Email.MessageCompressed := AlgoSynLZ.CompressToBytes(aBody);
    ...
    if not Email.FilterAndValidate(Rest.Orm,msg) then
      ...
      if Rest.Orm.Add(Email,true)=0 then
  ...
  try
    ...
    if (aTemplate='') or (ageInCache<>age) then begin
      ...
      if (aTemplate<>'') or (ageInCache<>0) then begin
        fCache.AddOrUpdate(aTemplateName, aTemplate, age);
        result := true;
      end;
    end else
